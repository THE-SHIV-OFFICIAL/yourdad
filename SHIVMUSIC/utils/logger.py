from ftmgram import Client, filters, enums
from ftmgram.enums import ParseMode, ButtonStyle
from ftmgram.types import InlineKeyboardMarkup, InlineKeyboardButton, ChatMemberUpdated
import random
...
# ====================================================
# AUTOPLAY LOGS  (#autoplay)
# ====================================================
async def autoplay_logs(client, chat_id, playing_title, upcoming_title=None):
    """Send a rich HTML #autoplay entry to the logger group."""
    if not LOGGER_ID:
        return
    try:
        bot = await client.get_me()
    ...
    bot_line = f"@{bot.username}" if bot and bot.username else "Music Bot"
    link_line = (
        f'<b>• ᴄʜᴀᴛ ʟɪɴᴋ :</b> <a href="{chat_link}">ᴏᴘᴇɴ ᴄʜᴀᴛ</a>'
        if chat_link
        else "<b>• ᴄʜᴀᴛ ʟɪɴᴋ :</b> ᴘʀɪᴠᴀᴛᴇ"
    )

    log_text = (
        "<blockquote><b><tg-emoji emoji-id=\"5217933090483098080\">🎶</tg-emoji> #autoplay ʟᴏɢ</b>\n\n"
        f"<b>• ʙᴏᴛ :</b> {bot_line}\n"
        f"<b>• ᴄʜᴀᴛ :</b> {chat_title} [<code>{chat_id}</code>]\n"
        f"<b>• ᴍᴇᴍʙᴇʀs :</b> {members_count}\n"
        f"<b>• ᴘʟᴀʏɪɴɢ :</b> {playing_title}\n"
        f"<b>• ᴜᴘᴄᴏᴍɪɴɢ :</b> {upcoming_title or 'ғᴇᴛᴄʜɪɴɢ ɴᴇxᴛ ᴛʀᴀᴄᴋ...'}\n"
        f"{link_line}</blockquote>"
    )
