        ...
        if not await is_active_chat(chat_id):
            ...
            try:
                ...
                if (
                    ...
                    return await message.reply_text(
                        "<blockquote><b><tg-emoji emoji-id=\"6100397162976252509\">🚫</tg-emoji> Assistant is Banned</b></blockquote>\n\n"
                        f"<blockquote><tg-emoji emoji-id=\"6100424015111787987\">📌</tg-emoji> <b>ID :</b> <code>{ub_id}</code>\n"
                        f"<tg-emoji emoji-id=\"5370546867786523009\">📝</tg-emoji> <b>Name :</b> {ub_name}\n"
                        f"<tg-emoji emoji-id=\"5258362837411045098\">👤</tg-emoji> <b>Username :</b> @{ub_username}</blockquote>\n\n"
                        "<blockquote><tg-emoji emoji-id=\"5409320020058584473\">🔓</tg-emoji> <b>The assistant account is banned in this chat. "
                        "Tap the button below to unban it (I need Ban Users permission), then play again.</b></blockquote>",
                        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(text="🔓 Unban Assistant", callback_data=f"clone_unban_assistant|{ub_id}")]]),
                    )
            except UserNotParticipant:
